...
```{r}
# select accuracy and reaction time columns only 
selected_files = list()
for(i in 1:length(files)){
  selected_files[[i]] = files[[i]] %>% select(ImageFile, study_same_key.keys, key_resp_10.keys,study_same_key.rt, key_resp_10.rt)
}
...
which(neutral_response_time > mean(neutral_response_time,na.rm = TRUE) + 3*sd(neutral_response_time, na.rm = TRUE))
```
